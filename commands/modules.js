        ...
        switch (platform) {
            case "twitch":
                ...
                if (command_args.length < 3) {
                    parent.functions.twitch_clientsay(channel, `[ADMIN] @${tags.username} Command usage: \`${command_args[0]} ${command_args[1]} (load|unload|reload) {module_type}/{module_name}\``)
                    return
                }
                ...
                if (/^(reloadm?|rlm?|reloadmodule)$/.test(command_args[2])) {
                    ...
                    if (command_args.length < 4) {
                        parent.functions.twitch_clientsay(channel, `[ADMIN] @${tags.username} Command usage: \`${command_args[0]} ${command_args[1]} ${command_args[1]} {module_type}/{module_name}\``)
                        return
                    }
                    ...
                    if (Object.keys(parent.commands).includes(command_args[3].toLowerCase().trim())) {
                        if (unload_module(command_args[3].toLowerCase().trim())) {
                            load_module(command_args[3].toLowerCase().trim()) ?
                                parent.functions.twitch_clientsay(channel, `[ADMIN] Reloaded the module: ${command_args[3]} (v${parent.commands[command_args[3]].version_added}), @${tags.username}`) :
                                parent.functions.twitch_clientsay(channel, `[ADMIN] Could not reload the module: ${command_args[3]}, @${tags.username}`)

                        } else {
                            parent.functions.twitch_clientsay(channel, `[ADMIN] Could not unload the module: ${command_args[3]}, @${tags.username}`)
                        }
                    } else {
                        parent.functions.twitch_clientsay(channel, `[ADMIN] That module was not found, or is not active @${tags.username}. Use \`${command_args[0]} loadmodule {module_type}/{module_name}\` instead.`)
                    }
                    return
                }
                ...
                if (/^(unloadm?|ulm?|unloadmodule)$/.test(command_args[2])) {
                    ...
                    if(command_args[3].toLowerCase().trim() === "modules"){
                        parent.functions.twitch_clientsay(channel, `[ADMIN] That module cannot be unloaded @${tags.username}.`)
                        return
                    }

                    if (Object.keys(parent.commands).includes(command_args[3].toLowerCase().trim())) {
                        unload_module(command_args[3].toLowerCase().trim()) ?
                            parent.functions.twitch_clientsay(channel, `[ADMIN] Unoaded the module: ${command_args[3]}, @${tags.username}`) :
                            parent.functions.twitch_clientsay(channel, `[ADMIN] Could not unload the module: ${command_args[3]}, @${tags.username}`)
                    } else {
                        parent.functions.twitch_clientsay(channel, `[ADMIN] That module was not found, or is not active @${tags.username}.`)
                    }
                    return
                }
